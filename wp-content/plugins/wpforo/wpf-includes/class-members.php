...
class wpForoMember{
 	...
 	private function add_profile($args){
		...
		return $this->wpforo->db->insert(
			...
			array(  'userid' => intval($userid), 
					...
					'about' => ( isset($about) ? wpforo_kses(trim($about), 'user_description') : '' ), 
	...
	function edit_profile($args){
		...
		if(isset($signature)){
			$fields['signature'] = wpforo_kses(trim($signature));
			$fields_types[] = '%s';
		}
		if(isset($about)){
			$fields['about'] = wpforo_kses(trim($about), 'user_description');
			$fields_types[] = '%s';
		}
		if(isset($occupation)){
			$fields['occupation'] = sanitize_text_field(trim($occupation));
			$fields_types[] = '%s';
		}
		if(isset($location)){
			$fields['location'] = sanitize_text_field(trim($location));
			$fields_types[] = '%s';
		}
		if(isset($timezone)){
		...
		if(isset($_FILES['avatar']) && !empty($_FILES['avatar']) && isset($_FILES['avatar']['name']) && $_FILES['avatar']['name']){
			...
			if(is_dir($avatar_dir)){
				if(move_uploaded_file($tmp_name, $avatar_path)) {
					...
					$this->wpforo->db->update($this->wpforo->db->prefix.'wpforo_profiles', array('avatar' => $upload_dir['baseurl']."/wpforo/avatars/".$avatar_fname), array('userid' => intval($userid)), array('%s'), array('%d'));
					$this->reset($userid);
				}
			} 
		}
		...
		if( $user = get_userdata($userid) ){
			...
			return $this->add_profile( 
							array(  'userid' => intval($userid), 
									...
									'about' => wpforo_kses(trim($about), 'user_description'), 
