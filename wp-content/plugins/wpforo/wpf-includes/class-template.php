...
//			TODO: move code to pm plugin
	...
	function icon($type, $item = array(), $echo = true ){
		...
		if(isset($item['type'])){
			...
			if( $item['closed'] && $item['type'] == 1 ){
				$icon['class'] = 'fa-lock';
				$icon['color'] = 'wpfcl-1';
				if($echo) { $status = true; echo implode(' ', $icon); } else{ return implode(' ', $icon); }
			}
			...
			if( $status ){
				...
				elseif( $type == 'topic' ){
					...
					elseif( $item['posts'] > 20 && $item['posts'] <= 100 ){
						$icon['class'] = 'fa-file-text';
						$icon['color'] = 'wpfcl-5';
					}
					elseif( $item['posts'] > 100 ){
						$icon['class'] = 'fa-file-text';
						$icon['color'] = 'wpfcl-5';
					}
					else{
				...
				if($echo) { echo implode(' ', $icon); } else{ return implode(' ', $icon); }
			}
		...
		if(isset($this->wpforo->current_object) && !empty($this->wpforo->current_object)){
			...
			if( is_user_logged_in() ){
				...
				$this->wpforo->menu['wpforo-register'] = array(
					...
					'attr' => ( isset($_GET['wpforo']) && $_GET['wpforo'] == 'register' ? ' class="wpforo-active"' : '' ),
					'submenues' => array()
				);
				$this->wpforo->menu['wpforo-login'] = array(
					'href' => wpforo_login_url(),
					'label' => wpforo_phrase('login', FALSE),
					'attr' => ( isset($_GET['wpforo']) && $_GET['wpforo'] == 'login' ? ' class="wpforo-active"' : '' ),
					'submenues' => array()
				);
			}
	...
	**/
	...
	public function copyright(){ ?>
		<span onclick='javascript:document.getElementById("bywpforo").style.display = "inline";document.getElementById("awpforo").style.display = "none";' id="awpforo"> <img align="absmiddle" class="wpdimg" src="<?php echo WPFORO_URL ?>/wpf-assets/images/wpforo-info.png" alt="wpForo"> </span><a id="bywpforo" target="_blank" href="http://wpforo.com/">&nbsp;<?php wpforo_phrase('Powered by') ?> wpForo version <?php echo esc_html($this->wpforo->version) ?></a>
		<?php
	}
