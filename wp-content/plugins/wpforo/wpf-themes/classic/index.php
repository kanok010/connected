			...
			if($template == 'search'){
				...
				if( $template == 'forum' || $template == 'topic' ) : ?>
					...
					<?php if(is_array($cats) && !empty($cats)) : ?>
						...
						<?php foreach($cats as $cat) : ?>
							<?php if( $wpforo->perm->forum_can( $cat['forumid'], 'vf' ) ): ?>
				...
				<?php if( $template == 'topic' ) : ?>
					<?php if( is_array($cats) && !empty($cats) && $cat['is_cat'] == 0 ) : ?>
						<?php $forum_data = $wpforo->forum->get_forum(array('slug'=> $forum_slug)); ?>
						
						<?php if(is_array($forum_data) && !empty($forum_data)) : ?>
							
							<?php if( $wpforo->perm->forum_can( $forum_data['forumid'], 'vf' ) ): ?>
		                        
		                        <div class="wpf-head-bar">
		                            <div class="wpf-head-bar-left">
                                    	<h2 id="wpforo-title"><?php echo esc_html($forum_data['title']) ?></h2>
                                        <div class="wpf-action-link">
                                        <?php if ( is_user_logged_in() ): ?>
                                            <?php 
                                            $args = array( "userid" => $wpforo->current_userid , "itemid" => $forum_data['forumid'], "type" => "forum" );
                                            $subscribe = $wpforo->sbscrb->get_subscribe( $args );
                                            if( isset( $subscribe['subid'] ) ): ?>
                                                <span class="wpf-unsubscribe-forum wpf-action" id="wpfsubscribe-<?php echo intval($forum_data['forumid']) ?>"><?php wpforo_phrase('Unsubscribe') ?></span> 
                                            <?php else: ?>
                                                <span class="wpf-subscribe-forum wpf-action" id="wpfsubscribe-<?php echo intval($forum_data['forumid']) ?>"><?php wpforo_phrase('Subscribe for new topics') ?></span> 
                                            <?php endif; ?>
                                        <?php endif; ?>
		                        	...
		                        	<?php if( $wpforo->perm->forum_can( $cat['forumid'], 'ct') ): ?>
								...
								<?php if( is_user_logged_in() && $wpforo->perm->forum_can( $cat['forumid'], 'ct' ) ) $wpforo->tpl->topic_form($forum_data['forumid']); ?>
				...
				<?php if( $template == 'post' ) : ?>
					<?php 
						$forum_data = $wpforo->forum->get_forum(array('slug'=> $forum_slug));
						
						if( is_array($forum_data) && !empty($forum_data) ) :
							
							if( $wpforo->perm->forum_can( $forum_data['forumid'], 'vt' ) ):
								$topic = $wpforo->topic->get_topic( array( 'slug'=> $topic_slug ) );
								if( is_array($topic) && !empty($topic) ) :
									...
									<?php if( is_array($posts) && !empty($posts) ) : ?>
										...
										<div class="wpf-head-bar">
                                            <h2 id="wpforo-title"><?php echo esc_html($topic['title']) ?>&nbsp;&nbsp;</h2>
                                            <?php if ( is_user_logged_in() ): ?>
                                                <div class="wpf-action-link">
                                                    <?php 
