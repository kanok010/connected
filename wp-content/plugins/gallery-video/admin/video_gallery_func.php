...
( 'New Video Gallery', '375', '600', 'on', 'cubeH', '4000', '1000', 'center', '1', '300', '4')";
   ...
   $query="SELECT * FROM ".$wpdb->prefix."huge_it_videogallery_galleries order by id ASC";
	...
	foreach($rowsldcc as $key=>$rowsldccs){
		...
		if($last_key == $key){
			header('Location: admin.php?page=videogallerys_huge_it_videogallery&id='.$rowsldccs->id.'&task=apply&huge_it_video_nonce='.$wp_video_nonce);
		}
	}	
}
...
$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."huge_it_videogallery_videos SET ordering = '%s' WHERE id = %d ", $rowplusorderspl, $rowplusorders->id));
}
...
{	
	...
	$script_cat = preg_replace('#<script(.*?)>(.*?)</script>#is', '', stripslashes($_POST["content"]));
			...
			if($_POST["name"] != ''){
			$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."huge_it_videogallery_galleries SET  name = '%'  WHERE id = %d ", $_POST["name"], $id));
			...
			$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."huge_it_videogallery_galleries SET  display_type = '%s'  WHERE id = %d ", $_POST["display_type"], $id));
			$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."huge_it_videogallery_galleries SET  content_per_page = '%s'  WHERE id = %d ", $_POST["content_per_page"], $id));
...
$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."huge_it_videogallery_videos SET  ordering = '%s'  WHERE ID = %d ", $_POST["order_by_".$rowimages->id.""], $rowimages->id));
$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."huge_it_videogallery_videos SET  link_target = '%s'  WHERE ID = %d ", $_POST["sl_link_target".$rowimages->id.""], $rowimages->id));
$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."huge_it_videogallery_videos SET  sl_url = '%s' WHERE ID = %d ", str_replace('%','__5_5_5__',$_POST["sl_url".$rowimages->id.""]),$rowimages->id));
$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."huge_it_videogallery_videos SET  name = '%s'  WHERE ID = %d ", str_replace('%','__5_5_5__',$_POST["titleimage".$rowimages->id.""]), $rowimages->id));
$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."huge_it_videogallery_videos SET  description = '%s'  WHERE ID = %d ", str_replace('%','__5_5_5__',$_POST["im_description".$rowimages->id.""]), $rowimages->id));
$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."huge_it_videogallery_videos SET  image_url = '%s'  WHERE ID = %d ", $_POST["imagess".$rowimages->id.""], $rowimages->id));
$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."huge_it_videogallery_videos SET  thumb_url = '%s'  WHERE ID = %d ", $_POST["thumbs".$rowimages->id.""], $rowimages->id));
}
}
